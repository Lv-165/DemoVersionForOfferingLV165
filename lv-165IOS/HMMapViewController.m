...
- (MKAnnotationView *)mapView:(MKMapView *)mapView
  ...
  if ([annotation isKindOfClass:[MKUserLocation class]]) {
    ...
    FBAnnotationClusterView *clusterAnnotationView =
      [[FBAnnotationClusterView alloc] initWithAnnotation:clusterAnnotation clusteringManager:_clusteringManager];
...
- (MKOverlayRenderer *)mapView:(MKMapView *)mapView
  ...
  if ([overlay isKindOfClass:[MKPolyline class]]) {
    ...
    //    //    specific stroke width. "Line cap" and "line join" control how
    //    the // start and end  of the shape looks and how the joins between
    //    path // components look (there    is only one component in this
    //    shape).
    ...
    //    Step 3: There is no step 3 (well, there is fill + stroke) // Fill this
    //    shape just like you did with the pie shapes. (lightGray and // black
    //    was used in Image 2 (above)).
    ...
    // MKOverlayPathRenderer *pathRenderer =   [[MKOverlayPathRenderer alloc]
    ...
    //        path using whatever new data your subclass has obtained. The miter
    //        limit helps you avoid spikes in paths that use the
    //        kCGLineJoinMiter join style. If the ratio of  the miter
    //        length—that is, the diagonal length of the miter join—to the line
    //        thickness exceeds the miter limit, the joint is converted to a
    //        bevel join. The default miter limit is 10, which results in the
    //        conversion of miters whose angle at the joint is less   than 11
    //        degrees
    // lineDashPhase
    // Property
    // The offset (in points) at which to start drawing the  dash pattern. Use
    // this property to start drawing a dashed line partway through a segment or
    // gap. For example, a phase value of 6 for the patter 5-2-3-2 would cause
    // drawing to begin in the middle of the first gap. The default value of
    // this property is 0.
    ...
    // *lineDashPattern
    // Discussion
    // The array contains one or more NSNumber objects that indicate the lengths
    // (measured in points) of the line segments and gaps in the pattern. The
    // values in the array alternate,  starting with the first line segment
    // length,  followed by the first gap length, followed by the second line
    // segment length, and so on. // This property is set to nil by default,
    // which indicates no line dash pattern.
    // Creating and Managing the Path // path
    // The path representing the overlay’s shape.
    // OBJECTIVE-C
    // @property CGPathRef path
    // Discussion
    // Getting the value of this property causes the path to be created (using
    // the createPath method) if it does not already exist. You can assign a
    // path object to this property explicitly. When assigning a new path object
    // to this property, the overlay renderer stores a strong reference to the
    // path you provide.

    // - createPath
    // Creates the path for the overlay.
    // The default implementation of this method does nothing. Subclasses should
    // override it and use it to create the CGPathRef data type to be used for
    // drawing. After creating the path, your  implementation should assign it
    ...
    //                                        - (void)invalidatePath
    // Discussion
    // Call this method when a change in the path information would require you
    // to recreate the overlay’s  path. This method sets the path property to
    // nil and tells the overlay renderer to redisplay its contents.
    //      Drawing the Path
    //  applyStrokePropertiesToContext:atZoomScale:
    // Applies the receiver’s current stroke-related drawing properties to the
    ...
    // Discussion
    // This is a convenience method for applying all of the drawing properties
    // used when     // stroking a path. This method applies the stroke color,
    // line width, line join, line cap, miter limit, line dash phase, and line
    // dash attributes to the specified graphics context. This method applies
    // the scale factor in the zoomScale parameter to the line width and line
    // dash pattern automatically so that lines scale appropriately.
    //
    // This method does not save the current graphics state  before applying the
    // new attributes. If you want to preserve the existing state, you must save
    // it yourself and restore it later when you finish // drawing.
    // applyFillPropertiesToContext:atZoomScale:
    // Applies the receiver’s current fill-related drawing properties to the
    // specified graphics context.

    //  (void)applyFillPropertiesToContext:(CGContextRef)context
    // atZoomScale:(MKZoomScale)zoomScale     // Parameters     // context
    // // The graphics context used to draw the view’s contents.
    // zoomScale // The current zoom scale     // used for drawing.
    // Discussion This is a convenience method for applying all of the drawing
    // properties used when filling a path. This method applies the current fill
    // color to  the specified graphicscontext.
    ...
    // Parameters path The path to fill. context The graphics context in which
    // to draw the path.Discussion You must set the current fill color before
    // calling this method. Typically you do this by calling the
    // applyFillPropertiesToContext:atZoomScale: method prior to drawing. If the
    // fillColor   property is  currently nil,  this method  does nothing.
  }
  return nil;
}
